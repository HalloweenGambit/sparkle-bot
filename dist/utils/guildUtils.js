...
export const loadGuild = async (id) => {
    try {
...
export const loadCompleteGuilds = async () => {
    try {
        ...
        const detailedGuildsPromises = await guilds.map((guild) => loadGuild(guild.id));
...
export const formatGuild = (guild) => {
    return {
        discordId: guild.id,
...
export const formatGuilds = (guilds) => {
...
// Find the guild
export const findGuild = async (discordId) => {
    try {
...
export const compareGuilds = (newData, oldData) => {
...
export const getChangedFields = (newData, oldData) => {
    const newGuild = formatGuild(newData);
    const changedFields = Object.entries(newGuild).reduce((changedFields, [key, value]) => {
