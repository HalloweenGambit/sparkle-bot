import { findChannel, formatGuildChannel, getChangedFields, } from '../../utils/channelUtils.js';
...
export const createChannel = async (channel) => {
    try {
        ...
        console.log(`Inserted channel ${newChannel.discordId}:${newChannel.channelName}`);
    }
    catch (error) {
        console.error(`Error inserting channel ${channel.id}:${channel.name}`, error);
        throw error;
    }
};
// Function to create multiple channels in the database
export const createChannels = async (channels) => {
    try {
        const db = await dbClient;
...
export const updateChannel = async (channel) => {
    try {
        ...
        console.log(`Updated channel: ${found.id}, changed fields: ${changedFields}`);
    }
    catch (error) {
        console.error(`Error updating channel ${channel.id}, ${channel.name}`, error);
    }
};
