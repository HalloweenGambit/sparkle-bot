...
export const channelSelector = async (guild) => {
    const guildChannels = await loadGuildChannels(guild);
    const rows = guildChannels.map((channel) => {
        ...
        const channelButton = new ButtonBuilder()
            ...
            .setStyle(ButtonStyle.Secondary);
        const row = new ActionRowBuilder().addComponents(selectButton, deselectButton, channelButton);
        return row;
    });
    return rows;
};
